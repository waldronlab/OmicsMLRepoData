...
write_csv(curated_all, file.path(extDir, "cBioPortal_curated_metadata.csv")) # 189,439 x 159
```
...
```{r}
dim(cbio_meta_release) # 189439 x 87
dim(cbio_meta_release_tidy) # 189439 x 83: removing non-availability info. Finalize the delimiter and use this as 'release'
```
...
write.csv(cbio_meta_release, fpath, row.names = FALSE) # 189,439 x 87
write.csv(cbio_meta_release_tidy, fpath_tidy, row.names = FALSE) # 189,439 x 83
