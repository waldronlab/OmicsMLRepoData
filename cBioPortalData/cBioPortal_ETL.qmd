...
```{r}
dim(cbio_meta_release)
dim(cbio_meta_release_tidy)
```
...
write.csv(cbio_meta_release_tidy, fpath_tidy, row.names = FALSE) # 189,439 x 57

## Sync the version in GCS bucket
library(AnVILGCP)
avcopy(fpath, "gs://omics_ml_repo/cBioPortal_curated_metadata_release_tidy.csv", platform = gcp())
```
