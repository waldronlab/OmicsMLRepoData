---
title: "Harmonize bodysite information in cbio data"
author:
  - Sehyun Oh
  - Kai Gravel-Pucillo
...
This file curates information from 37 original columns to produce a curated 
...
original_cols <- unlist(strsplit(pot_col_map$original_col[8], ";"))
...
summary_bodysite_cols <- data.frame(colnames(bodysite_cols_cbio[,2:38]))

# Iterate through these columns to create a summary of each column's completeness
for (col in 1:nrow(summary_bodysite_cols)){
  summary_bodysite_cols$completeness[col] <- (nrow(bodysite_cols_cbio) - length(which(is.na(bodysite_cols_cbio[,col]))))/nrow(bodysite_cols_cbio)
...
inaccurating mapping of these terms, we can change the original value in certain
...
for (i in 1:nrow(bodysite_cols_cbio)){
  bodysite_cols_cbio$merged[i] <- toupper(paste(bodysite_cols_cbio[i,2:38], collapse="<;>"))
}
```
...
bodysite_cols_cbio$merged <- gsub("<;>8X8", "", bodysite_cols_cbio$merged)
bodysite_cols_cbio$merged <- gsub("<;>+", "<;>", bodysite_cols_cbio$merged)
bodysite_cols_cbio$merged <- gsub("^<;>", "", bodysite_cols_cbio$merged)
bodysite_cols_cbio$merged <- gsub("<;>$", "", bodysite_cols_cbio$merged)
...
```{r}
# Count the number of unique values in the merged column (692)
merged_vals <- list()
...
write.csv(unique_merged_vals, 
          file= file.path(proj_dir, "bodysite_cbio_unique_vals.csv"), 
...
`bodysite_ontology_map`.
```{r import_maps, eval=FALSE}
# import bodysite ontology map
mapped_terms <- read_sheet(ss, sheet = "bodysite_ontology_map")

# import bodysite ontology map
mapped_terms <-read.csv(file = file.path(proj_dir, "cBioPortal_bodysite_ontology_map.csv"), header=T)
```


# Curating Bodysite Columns
...
for (x in 1:length(bodysite_cols_cbio$merged)){
  ...
  bodysite_cols_cbio$curated_bodysite[x] <- rmv_xtra_semis(paste(as.list(unique(new_terms)), collapse= "<;>"))
  bodysite_cols_cbio$curated_bodysite_ontology_term_id[x] <- rmv_xtra_semis(paste(as.list(unique(new_term_ids)), collapse= "<;>"))
...
for (i in 1:nrow(bodysite_cols_cbio)){
  bodysite_cols_cbio$source_columns[i] <- paste(colnames(bodysite_cols_cbio[,2:38][which(!is.na(bodysite_cols_cbio[i,2:38]))]), collapse="<;>")
}

# Create a curated dataframe
curated_bodysite <- bodysite_cols_cbio[,c(1, 39:42)]

# Rename the columns for accuracy & specificity
colnames(curated_bodysite)[1:2] <- c("curation_id", "original_bodysite")
...
# export to GitHub
write.csv(curated_bodysite, file = file.path(proj_dir, "curated_bodysite.csv"), 
          row.names=F)
```