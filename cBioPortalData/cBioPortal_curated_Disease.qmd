...
proj_dir <- "C:\\Users\\Owner\\Desktop\\CUNY Internship\\cbio\\"
```
...
pot_col_map <- read.csv(file=file.path(proj_dir, "cBioPortal_potential_columns_map.csv"))

# Get list of all columns to combine into new Disease column
original_cols <- unlist(strsplit(pot_col_map$Original.Columns[1], ";"))
...
for(col in colnames(binary_value_cols[,2:15])){
...
disease_cols_cbio[,binary_cols] <- binary_value_cols[,2:15]
```
...
for (i in 1:nrow(disease_cols_cbio)){
  disease_cols_cbio$merged[i] <- toupper(paste(disease_cols_cbio[i,2:76], collapse="<;>"))
}
```
...
disease_cols_cbio$merged <- gsub("<;>8X8", "", disease_cols_cbio$merged)
disease_cols_cbio$merged <- gsub("<;>+", "<;>", disease_cols_cbio$merged)
disease_cols_cbio$merged <- gsub("^<;>", "", disease_cols_cbio$merged)
disease_cols_cbio$merged <- gsub("<;>$", "", disease_cols_cbio$merged)
...
```{r}
# Count the number of unique values in the merged column (2125 excluding icd codes)
merged_vals <- list()
...
pop_ontology_map <- read_sheet(ss, sheet = "disease_ontology_map")

# import curated and source columns map
curated_source_map <- read_sheet(ss, sheet = "curated_columns_map")

# import disease ontology map
mapped_terms <-read.csv(file = file.path(proj_dir, "cBioPortal_disease_ontology_map.csv"), header=T)

# import curated and source columns map
curated_columns_map <-read.csv(file = file.path(proj_dir, "cBioPortal_curated_columns_map.csv"), header=T)
```
...
cols_to_curate <- hash()
cols_to_curate[["specimen_type"]] <- curated_columns_map$original_column[which(curated_columns_map$curated_column=="specimen_type")]
cols_to_curate[["disease"]] <- curated_columns_map$original_column[which(curated_columns_map$curated_column=="disease")]
cols_to_curate[["cancer_type"]] <- curated_columns_map$original_column[which(curated_columns_map$curated_column=="cancer_type")]
cols_to_curate[["cancer_type_detailed"]] <- curated_columns_map$original_column[which(curated_columns_map$curated_column=="cancer_type_detailed")]
cols_to_curate[["cancer_subtype"]] <- curated_columns_map$original_column[which(curated_columns_map$curated_column=="cancer_subtype")]
cols_to_curate[["metastasis"]] <- curated_columns_map$original_column[which(curated_columns_map$curated_column=="metastasis")]
cols_to_curate[["acronym"]] <- curated_columns_map$original_column[which(curated_columns_map$curated_column=="acronym")]
cols_to_curate[["cancer_status"]] <- curated_columns_map$original_column[which(curated_columns_map$curated_column=="cancer_status")]
```
...
for (col in 2:9){
  for (x in 1:length(disease_curated_cols[,col])){
    ...
    disease_curated_cols[x,col+8] <- rmv_xtra_semis(paste(as.list(unique(new_terms)), collapse= "<;>"))
    disease_curated_cols[x,col+16] <- rmv_xtra_semis(paste(as.list(unique(new_term_ids)), collapse= "<;>"))
    if(x %% 5000==0){print(x)}
  }
...
colnames(disease_curated_cols)[1:9] <- c("curation_id", "original_acronym", 
    "original_cancer_status", "original_cancer_subtype", "original_cancer_type", 
    "original_cancer_type_detailed", "original_disease", "original_metastasis", 
