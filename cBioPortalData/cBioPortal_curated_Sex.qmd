...
This file curates information from 12 original columns to produce a curated 
...
proj_dir <- "C:\\Users\\Owner\\Desktop\\CUNY Internship\\cbio\\Sex\\"
```
...
pot_col_map <- read.csv(file=file.path(proj_dir, "potential_sex_columns_map.csv"))
...
F_indicators <- c("DMETS_DX_OVARY", "DMETS_DX_FEMALE_GENITAL")

# Replace "Yes" values with appropriate Female/Male value
sex_cols_cbio[,F_indicators] <- data.frame(lapply(sex_cols_cbio[,F_indicators], function(x) gsub("^Yes$", "Female", x)))
sex_cols_cbio[,"TESTICULAR_INVOLVEMENT"] <- data.frame(lapply(sex_cols_cbio[,"TESTICULAR_INVOLVEMENT"], function(x) gsub("^Yes$", NA, x)))
sex_cols_cbio[,"DMETS_DX_MALE_GENITAL"] <- data.frame(lapply(sex_cols_cbio[,"DMETS_DX_MALE_GENITAL"], function(x) gsub("^Yes$", "Male", x)))

# Replace "No" values with appropriate Female/Male value
sex_cols_cbio[,c(F_indicators, "DMETS_DX_MALE_GENITAL", "TESTICULAR_INVOLVEMENT")] <- data.frame(lapply(sex_cols_cbio[,c(F_indicators, "DMETS_DX_MALE_GENITAL", "TESTICULAR_INVOLVEMENT")], function(x) gsub("^No$", NA, x)))
...
for (val in c("Unknown", "ABSTRACTION_PENDING", "U")){
  sex_cols_cbio <- data.frame(lapply(sex_cols_cbio, function(x) gsub(val, NA, x)))
}
...
for (i in 1:nrow(sex_cols_cbio)){
  sex_cols_cbio$merged[i] <- toupper(paste(sex_cols_cbio[i,2:7], collapse=";"))
}
```
...
mapped_terms <- read_sheet(ss, sheet = "sex_ontology_map")

# import bodysite ontology map
mapped_terms <-read.csv(file = file.path(proj_dir, "cBioPortal_sex_ontology_map.csv"), header=T)
```
...
for (i in 1:nrow(sex_cols_cbio)){
  sex_cols_cbio$source_columns[i] <- paste(colnames(sex_cols_cbio[,2:7][which(!is.na(sex_cols_cbio[i,2:7]))]), collapse=";")
}

# Create a curated dataframe
curated_sex <- sex_cols_cbio[,c(1, 8:11)]
...
error has affected the `luad_msk_npjpo_2021` study, as 303 of the curated_sex 
...
however if it is present in addition studies then we know the curated_sex value 
