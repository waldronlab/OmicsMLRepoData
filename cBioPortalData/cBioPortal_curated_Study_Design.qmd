...
mapped_terms <- read_sheet(ss, sheet = "cBioPortal_target_phenotype_map")

# import bodysite ontology map
mapped_terms <-read.csv(file = file.path(proj_dir, "cBioPortal_study_design_map.csv"), header=T)
mapped_terms_x <-read.csv(file = file.path(proj_dir, "cBioPortal_target_phenotype_map.csv"), header=T)
```
...
for(x in 1:length(studydesign_cols_cbio$study_id)){
  ...
  studydesign_cols_cbio$curated_target_phenotype[x] <- paste(mapped_terms_x$curated_ontology[which(studydesign_cols_cbio$study_id[x]==mapped_terms_x$study_id)])
  studydesign_cols_cbio$curated_target_phenotype_term_id[x] <- paste(mapped_terms_x$curated_ontology_term_id[which(studydesign_cols_cbio$study_id[x]==mapped_terms_x$study_id)])
