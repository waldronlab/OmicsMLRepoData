...
cbio$`DIAGNOSIS.AGE` <- cbio$`DIAGNOSIS AGE`
...
Now we can merge these values into a single column with 
...
for (curcol in keys(cols_to_curate)){
  ...
  for (i in 1:nrow(age_cols_cbio)){
    age_curated_cols[i,curcol] <- toupper(paste(na.omit(as.vector(unlist(age_cols_cbio[i,cols_to_curate[[curcol]]]))), collapse=";"))
  }
  print(curcol)
}
```
...
age_cols_cbio$AGE_AT_DIAGNOSIS[which(cbio$studyId=="nbl_msk_2023")] <- ""
...
for (curcol in keys(cols_to_curate)){
  # Merge all relevant rows
  for (i in 1:nrow(age_cols_cbio)){
    age_curated_cols[i,paste("curated", curcol, sep="_")] <- toupper(paste(na.omit(as.vector(unlist(age_cols_cbio[i,cols_to_curate[[curcol]]]))), collapse=";"))
  }
  print(curcol)
}
