---
title: "Harmonize disease acronym information in cbio data"
author:
  - Sehyun Oh
  - Kai Gravel-Pucillo
...
disease acronym data with the cbioportal dataset.
This file curates information from 93 original columns to produce a curated 
column relating to patients' disease acronym status and condition.
...
write.csv(unique_merged_vals, 
          file= file.path(proj_dir, "disease_acronym_cbio_unique_vals.csv"), 
...
`cBioPortal_disease_acronym_map.csv` file, and to map our original columns to their corresponding
...
# mapped_terms <- read_sheet(ss, sheet = "cBioPortal_disease_acronym_map.csv")

# import disease ontology map
mapped_terms <-read.csv(file = file.path(proj_dir, "cBioPortal_disease_acronym_map.csv"), header=T)
```



# Curating disease_acronym Column
...
for (x in 1:length(disease_cols_cbio$merged)){
  ...
  disease_cols_cbio$curated_acronym[x] <- format_list(as.list(unlist(strsplit(new_terms, "<;>"))), "<;>")
...
for (i in 1:nrow(disease_cols_cbio)){
  disease_cols_cbio$curated_acronym_source[i] <- paste(colnames(disease_cols_cbio[,2:92][which(!is.na(disease_cols_cbio[i,2:92]))]), collapse="<;>")
}

# Remove extraneous delimiters
disease_cols_cbio$curated_acronym <- gsub("(<;>)+", "<;>", disease_cols_cbio$curated_acronym)

# Create a curated dataframe
curated_acronym <- disease_cols_cbio[,c(1, 93:95)]

# Rename the columns for accuracy & specificity
colnames(curated_acronym)[1:2] <- c("curation_id", "original_acronym")

# Replace empty values from curated columns with "NA"
curated_acronym <- data.frame(lapply(curated_acronym, 
...
# export to GitHub
write.csv(curated_acronym, file = file.path(proj_dir, "curated_disease_acronym.csv"), 
          row.names=F)
```