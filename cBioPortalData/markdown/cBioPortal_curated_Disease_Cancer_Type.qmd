---
title: "Harmonize disease cancer_type information in cbio data"
author:
  - Sehyun Oh
  - Kai Gravel-Pucillo
...
disease cancer_type data with the cbioportal dataset.
This file curates information from 93 original columns to produce a curated 
column relating to patients' disease cancer_type status and condition.
...
url <- "https://docs.google.com/spreadsheets/d/1dsbPqDNgzu4v7y2wzPIUP6GwoNTnArLT_SYl0q7K318/edit?usp=sharing"
...
write.csv(unique_merged_vals, 
          file= file.path(proj_dir, "disease_cancer_type_cbio_unique_vals.csv"), 
...
`cBioPortal_disease_cancer_type_map.csv` file, and to map our original columns to their corresponding
...
# mapped_terms <- read_sheet(ss, sheet = "cBioPortal_disease_cancer_type_map.csv")

# import disease ontology map
mapped_terms <-read.csv(file = file.path(proj_dir, "cBioPortal_disease_cancer_type_map.csv"), header=T)
```



# Curating disease_cancer_type Column
...
for (x in 1:length(disease_cols_cbio$merged)){
  ...
  disease_cols_cbio$curated_cancer_type[x] <- format_list(as.list(unlist(strsplit(new_terms, "<;>"))), "<;>")
  disease_cols_cbio$curated_cancer_type_ontology_term_id[x] <- format_list(as.list(unlist(strsplit(new_term_ids, "<;>"))), "<;>")
...
for (i in 1:nrow(disease_cols_cbio)){
  disease_cols_cbio$curated_cancer_type_source[i] <- paste(colnames(disease_cols_cbio[,2:92][which(!is.na(disease_cols_cbio[i,2:92]))]), collapse="<;>")
}

# Remove extraneous delimiters
disease_cols_cbio$curated_cancer_type <- gsub("(<;>)+", "<;>", disease_cols_cbio$curated_cancer_type)
disease_cols_cbio$curated_cancer_type_ontology_term_id <- gsub("(<;>)+", "<;>", disease_cols_cbio$curated_cancer_type_ontology_term_id)

# Create a curated dataframe
curated_cancer_type <- disease_cols_cbio[,c(1, 93:96)]

# Rename the columns for accuracy & specificity
colnames(curated_cancer_type)[1:2] <- c("curation_id", "original_cancer_type")

# Replace empty values from curated columns with "NA"
curated_cancer_type <- data.frame(lapply(curated_cancer_type, 
...
# export to GitHub
write.csv(curated_cancer_type, file = file.path(proj_dir, "curated_disease_cancer_type.csv"), 
          row.names=F)
```