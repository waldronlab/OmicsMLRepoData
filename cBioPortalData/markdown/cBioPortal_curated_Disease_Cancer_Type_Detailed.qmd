---
title: "Harmonize disease cancer_type_detailed information in cbio data"
author:
  - Sehyun Oh
  - Kai Gravel-Pucillo
...
disease cancer_type_detailed data with the cbioportal dataset.
This file curates information from 93 original columns to produce a curated 
column relating to patients' disease cancer_type_detailed status and condition.
...
url <- "https://docs.google.com/spreadsheets/d/1gUIZBJSO3lINmTpVCvwCZEAbl0H_df1z94TLfbN-7ZU/edit?usp=sharing"
...
Now that all columns include descriptive values, we can  
...
Next, we can remove the pipe delimiter from all values to 
...
write.csv(unique_merged_vals, 
          file= file.path(proj_dir, "disease_cancer_type_detailed_cbio_unique_vals.csv"), 
...
`cBioPortal_disease_cancer_type_detailed_map.csv` file, and to map our original columns to their corresponding
...
# mapped_terms <- read_sheet(ss, sheet = "cBioPortal_disease_cancer_type_detailed_map.csv")

# import disease ontology map
mapped_terms <-read.csv(file = file.path(proj_dir, "cBioPortal_disease_cancer_type_detailed_map.csv"), header=T)
```



# Curating disease_cancer_type_detailed Column
...
for (x in 1:length(disease_cols_cbio$merged)){
  ...
  disease_cols_cbio$curated_cancer_type_detailed[x] <- format_list(as.list(unlist(strsplit(new_terms, "<;>"))), "<;>")
  disease_cols_cbio$curated_cancer_type_detailed_ontology_term_id[x] <- format_list(as.list(unlist(strsplit(new_term_ids, "<;>"))), "<;>")
  if(x %% 10000==0){print(x)}
}
```

Next we can condense repetitive values into a single curated term: 
```{r}
# Load in the condensed_concepts table
condensed_curations <- read_delim(file.path(proj_dir,"cBioPortal_disease_cancer_type_detailed_condensed_values_map.csv"), delim = ",")

# Expand the essential_concepts into list form
condensed_curations <- condensed_curations %>%
  mutate(essential_concepts_split = str_split(essential_concepts, fixed("<;>")),
         essential_ontologies_split = str_split(essential_ontology_term_ids, fixed("<;>")),
         essential_concepts_size  = lengths(essential_concepts_split)) %>%
  arrange(desc(essential_concepts_size))

# Convert curated values into lists
disease_cols_cbio <- disease_cols_cbio %>%
  mutate(curated_split = str_split(curated_cancer_type_detailed, fixed("<;>")),
         curated_onto_split = str_split(curated_cancer_type_detailed_ontology_term_id, fixed("<;>")))

# Iterate through all rows of the cBioPortal data
disease_cols_cbio <- disease_cols_cbio %>% rowwise() %>%
  mutate(updated_values = {
      current_vals <- curated_split
      current_vals <- unlist(current_vals)
      # Iterate through condensed_curations to prep value mapping
      for (i in 1:nrow(condensed_curations)){
        essential_concepts_vals <- condensed_curations$essential_concepts_split[[i]]
        condensed_val <- condensed_curations$condensed_concept[[i]]
        # If all essential values are in the curated column, replace with condensed value
        if (all(essential_concepts_vals %in% current_vals)){
          current_vals <- unique(c(setdiff(current_vals, essential_concepts_vals), condensed_val))
        }
      }
      # Collapse remaining values on delimiter
      format_list(current_vals, "<;>")
    }) %>% ungroup()
disease_cols_cbio <- disease_cols_cbio %>% rowwise() %>%
  mutate(updated_ontos = {
      current_onto_vals <- curated_onto_split
      current_onto_vals <- unlist(current_onto_vals)
      # Iterate through condensed_curations to prep ontology value mapping
      for (i in 1:nrow(condensed_curations)){
        essential_ontologies_vals <- condensed_curations$essential_ontologies_split[[i]]
        condensed_onto_val <- condensed_curations$condensed_ontology_term_id[[i]]
        # If all essential values are in the curated column, replace with condensed value
        if (all(essential_ontologies_vals %in% current_onto_vals)){
          current_onto_vals <- unique(c(setdiff(current_onto_vals, essential_ontologies_vals), condensed_onto_val))
        }
      }
      # Collapse remaining values on delimiter
      format_list(current_onto_vals, "<;>")
    }) %>%  ungroup() 
disease_cols_cbio <- disease_cols_cbio %>% select(-curated_split, -curated_onto_split, -curated_cancer_type_detailed, -curated_cancer_type_detailed_ontology_term_id)
disease_cols_cbio <- disease_cols_cbio %>% rename(curated_cancer_type_detailed = updated_values, curated_cancer_type_detailed_ontology_term_id = updated_ontos)
```
...
for (i in 1:nrow(disease_cols_cbio)){
  disease_cols_cbio$curated_cancer_type_detailed_source[i] <- paste(colnames(disease_cols_cbio[,2:92][which(!is.na(disease_cols_cbio[i,2:92]))]), collapse="<;>")
}

# Remove extraneous delimiters
disease_cols_cbio$curated_cancer_type_detailed <- gsub("(<;>)+", "<;>", disease_cols_cbio$curated_cancer_type_detailed)
disease_cols_cbio$curated_cancer_type_detailed_ontology_term_id <- gsub("(<;>)+", "<;>", disease_cols_cbio$curated_cancer_type_detailed_ontology_term_id)

# Create a curated dataframe
curated_cancer_type_detailed <- disease_cols_cbio[,c(1, 93:96)]

# Rename the columns for accuracy & specificity
colnames(curated_cancer_type_detailed)[1:2] <- c("curation_id", "original_cancer_type_detailed")

# Replace empty values from curated columns with "NA"
curated_cancer_type_detailed <- data.frame(lapply(curated_cancer_type_detailed, 
...
# export to GitHub
write.csv(curated_cancer_type_detailed, file = file.path(proj_dir, "curated_disease_cancer_type_detailed.csv"), 
          row.names=F)
```