---
title: "Harmonize disease histological_subtype and molecular_subtype information in cbio data"
author:
  - Sehyun Oh
  - Kai Gravel-Pucillo
...
disease histological_subtype and molecular_subtype data with the cbioportal dataset.
This file curates information from 93 original columns to produce a curated 
column relating to patients' disease histological_subtype and molecular_subtype 
...
url <- "https://docs.google.com/spreadsheets/d/1ULYSi3ijVeqms0lmR1tbwwcGQhxAEfZIUwzxYVhpQ3Y/edit?usp=sharing"
...
a single column with each distinct value separated by a special 3-character 
delimiter ("<;>"). This delimiter was implemented so as to preserve curated 
ontology terms containing semicolons, which are the main delimiter utilized in 
the broader curation.
```{r}
# Merge all relevant rows
for (i in 1:nrow(disease_cols_cbio)){
  disease_cols_cbio$merged[i] <- toupper(paste(na.omit(as.vector(unlist(disease_cols_cbio[i,2:92]))), collapse="<;>"))
}
```

Next, we can remove the pipe delimiter from all values to prevent interpretation 
of the pipe as a regex character during queries:
```{r}
...
```{r}
# Clarify "superficial" (bladder cancer vs melanoma)
for(x in 1:length(disease_cols_cbio$merged)){
  vals <- unlist(strsplit(disease_cols_cbio$merged[x], "<;>"))
  if("SKIN CANCER, NON-MELANOMA" %in% vals & "SUPERFICIAL" %in% vals){
    vals[vals == "SUPERFICIAL"] <- "SUPERFICIAL_S"
    disease_cols_cbio$merged[x] <- paste(as.list(vals), collapse= "<;>")
    print(disease_cols_cbio$merged[x])
  }else if("BLADDER CANCER" %in% vals & "SUPERFICIAL" %in% vals){
    vals[vals == "SUPERFICIAL"] <- "SUPERFICIAL_B"
    disease_cols_cbio$merged[x] <- paste(as.list(vals), collapse= "<;>")
    print(disease_cols_cbio$merged[x])
  }
}
...
write.csv(unique_merged_vals, 
          file= file.path(proj_dir, "disease_cancer_subtype_cbio_unique_vals.csv"), 
...
`cBioPortal_disease_cancer_subtype_map.csv` file, and to map our original columns to their corresponding
...
# mapped_terms <- read_sheet(ss, sheet = "cBioPortal_disease_cancer_subtype_map.csv")

# import disease ontology map
mapped_terms <-read.csv(file = file.path(proj_dir, "cBioPortal_disease_cancer_subtype_map.csv"), header=T)
```



# Curating disease_cancer_subtype Column
...
```{r}
# Separate terms by classification
histo_terms <- mapped_terms %>% filter(Classification=="Histological")
molec_terms <- mapped_terms %>% filter(Classification=="Molecular")
...
for (x in 1:length(disease_cols_cbio$merged)){
  ...
  new_terms_m <- list()
  new_term_ids_m <- list()
  # Search for replacement terms in the histological ontology map
  new_terms_h <- lapply(original_terms, function(y) format_list(list_drop_empty(as.list(histo_terms$curated_ontology[grep(paste("^",y,"$",sep=""), histo_terms$original_value, fixed=F)])), "<;>"))
  new_term_ids_h <- lapply(original_terms, function(y) format_list(list_drop_empty(as.list(histo_terms$curated_ontology_term_id[grep(paste("^",y,"$",sep=""), histo_terms$original_value, fixed=F)])), "<;>"))
  new_terms_h <- format_list(list_drop_empty(new_terms_h), "<;>")
  new_term_ids_h <- format_list(list_drop_empty(new_term_ids_h), "<;>")
  # Search for replacement terms in the molecular ontology map
  new_terms_m <- lapply(original_terms, function(y) format_list(list_drop_empty(as.list(molec_terms$curated_ontology[grep(paste("^",y,"$",sep=""), molec_terms$original_value, fixed=F)])), "<;>"))
  new_term_ids_m <- lapply(original_terms, function(y) format_list(list_drop_empty(as.list(molec_terms$curated_ontology_term_id[grep(paste("^",y,"$",sep=""), molec_terms$original_value, fixed=F)])), "<;>"))
  new_terms_m <- format_list(list_drop_empty(new_terms_m), "<;>")
  new_term_ids_m <- format_list(list_drop_empty(new_term_ids_m), "<;>")
  # Concatenate new lists on ";" delimiter to create curated value
  disease_cols_cbio$curated_histological_subtype[x] <- format_list(as.list(unlist(strsplit(new_terms_h, "<;>"))), "<;>")
  disease_cols_cbio$curated_histological_subtype_ontology_term_id[x] <- format_list(as.list(unlist(strsplit(new_term_ids_h, "<;>"))), "<;>")
  disease_cols_cbio$curated_molecular_subtype[x] <- format_list(as.list(unlist(strsplit(new_terms_m, "<;>"))), "<;>")
  disease_cols_cbio$curated_molecular_subtype_ontology_term_id[x] <- format_list(as.list(unlist(strsplit(new_term_ids_m, "<;>"))), "<;>")
...
for (i in 1:nrow(disease_cols_cbio)){
  disease_cols_cbio$curated_histological_subtype_source[i] <- paste(colnames(disease_cols_cbio[,2:92][which(!is.na(disease_cols_cbio[i,2:92]))]), collapse="<;>")
  disease_cols_cbio$curated_molecular_subtype_source[i] <- paste(colnames(disease_cols_cbio[,2:92][which(!is.na(disease_cols_cbio[i,2:92]))]), collapse="<;>")
}

# Remove extraneous delimiters
disease_cols_cbio$curated_histological_subtype <- gsub("(<;>)+", "<;>", disease_cols_cbio$curated_histological_subtype)
disease_cols_cbio$curated_histological_subtype_ontology_term_id <- gsub("(<;>)+", "<;>", disease_cols_cbio$curated_histological_subtype_ontology_term_id)
disease_cols_cbio$curated_molecular_subtype <- gsub("(<;>)+", "<;>", disease_cols_cbio$curated_molecular_subtype)
disease_cols_cbio$curated_molecular_subtype_ontology_term_id <- gsub("(<;>)+", "<;>", disease_cols_cbio$curated_molecular_subtype_ontology_term_id)

# Create a curated dataframe
curated_histological_subtype <- disease_cols_cbio[,c(1, 93:95,98)]
curated_molecular_subtype <- disease_cols_cbio[,c(1, 93,96,97,99)]

# Rename the columns for accuracy & specificity
colnames(curated_histological_subtype)[1:2] <- c("curation_id", "original_histological_subtype")
colnames(curated_molecular_subtype)[1:2] <- c("curation_id", "original_molecular_subtype")

# Replace empty values from curated columns with "NA"
curated_histological_subtype <- data.frame(lapply(curated_histological_subtype, 
                                      function(x) gsub("^$", NA, x)))
curated_molecular_subtype <- data.frame(lapply(curated_molecular_subtype, 
                                      function(x) gsub("^$", NA, x)))
```
...
# export to GitHub
write.csv(curated_histological_subtype, file = file.path(proj_dir, "curated_disease_histological_subtype.csv"), 
          row.names=F)
write.csv(curated_molecular_subtype, file = file.path(proj_dir, "curated_disease_molecular_subtype.csv"), 
          row.names=F)
```