---
title: "Harmonize disease specimen_type information in cbio data"
author:
  - Sehyun Oh
  - Kai Gravel-Pucillo
...
This file curates information from 95 original columns to produce a curated 
...
url <- "https://docs.google.com/spreadsheets/d/1RPjLdmZLibU17uutOKMBp1XBVqmQ7g3H8Ub1FaRAFBA/edit?gid=0#gid=0"
...
disease_columns <- select(cbio, contains(c("disease", "cancer", "tumor", "type", "diagnosis", "ilc", "class", "sample", "specimen"))) %>%
...
original_cols <- unlist(strsplit(pot_col_map$original_columns[34], ";"))
...
for (i in 1:nrow(disease_cols_cbio)){
  disease_cols_cbio$merged[i] <- toupper(paste(na.omit(as.vector(unlist(disease_cols_cbio[i,2:96]))), collapse="<;>"))
}
```
...
write.csv(unique_merged_vals, 
          file= file.path(proj_dir, "disease_specimen_type_cbio_unique_vals.csv"), 
...
`cBioPortal_disease_specimen_type_map.csv` file, and to map our original columns to their corresponding
...
# mapped_terms <- read_sheet(ss, sheet = "cBioPortal_disease_specimen_type_map.csv")

# import disease ontology map
mapped_terms <-read.csv(file = file.path(proj_dir, "cBioPortal_disease_specimen_type_map.csv"), header=T)
```



# Curating disease_specimen_type Column
...
for (x in 1:length(disease_cols_cbio$merged)){
  ...
  disease_cols_cbio$curated_specimen_type[x] <- format_list(as.list(unlist(strsplit(new_terms, "<;>"))), "<;>")
  disease_cols_cbio$curated_specimen_type_ontology_term_id[x] <- format_list(as.list(unlist(strsplit(new_term_ids, "<;>"))), "<;>")
...
for (i in 1:nrow(disease_cols_cbio)){
  disease_cols_cbio$curated_specimen_type_source[i] <- paste(colnames(disease_cols_cbio[,2:96][which(!is.na(disease_cols_cbio[i,2:96]))]), collapse="<;>")
}

# Remove extraneous delimiters
disease_cols_cbio$curated_specimen_type <- gsub("(<;>)+", "<;>", disease_cols_cbio$curated_specimen_type)
disease_cols_cbio$curated_specimen_type_ontology_term_id <- gsub("(<;>)+", "<;>", disease_cols_cbio$curated_specimen_type_ontology_term_id)

# Create a curated dataframe
curated_specimen_type <- disease_cols_cbio[,c(1, 97:100)]

# Rename the columns for accuracy & specificity
colnames(curated_specimen_type)[1:2] <- c("curation_id", "original_specimen_type")

# Replace empty values from curated columns with "NA"
curated_specimen_type <- data.frame(lapply(curated_specimen_type, 
...
# export to GitHub
write.csv(curated_specimen_type, file = file.path(proj_dir, "curated_disease_specimen_type.csv"), 
          row.names=F)
```