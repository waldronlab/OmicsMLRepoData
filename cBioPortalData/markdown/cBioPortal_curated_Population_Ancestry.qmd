...
`cBioPortal_population_ancestry_map`.
```{r import_maps, eval=FALSE}
# import population ancestry ontology map
pop_ontology_map <- read_sheet(ss, sheet = "cBioPortal_population_ancestry_map")

# import the pop ontology map
pop_ontology_map <- read.csv(file = file.path(proj_dir, "cBioPortal_population_ancestry_map.csv"),
                           sep = ",", header = TRUE)
```
...
write.csv(chk, file = file.path(proj_dir, "population_ancestry_discrepancies.csv"),
      row.names = FALSE)

# Export a list of the unique discrepancy value combos to manually create a map
pop_a_discreps <- data.frame(unique(chk$values))
write.csv(pop_a_discreps, 
          file = file.path(proj_dir, "cBioPortal_pop_ancestry_discrepancies.csv"),
          row.names = FALSE)
```
...
pop_ancestry_discrep <- read.csv(file=file.path(proj_dir, "cBioPortal_population_ancestry_discrepancies.csv"))
