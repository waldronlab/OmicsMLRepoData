...
for (i in 1:nrow(sex_cols_cbio)){
  sex_cols_cbio$curated_sex_source[i] <- paste(colnames(sex_cols_cbio[,2:7][which(!is.na(sex_cols_cbio[i,2:7]))]), collapse=";")
}
...
for (pat in chk1$patient_id){
  ...
  curated_sex$curated_pop_ancestry_ontology_term_id[which(cbio$patientId==pat)] <- standard_id
}
```
