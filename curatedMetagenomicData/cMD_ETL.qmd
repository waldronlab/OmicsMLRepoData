...
cmd_etl_dir <- file.path(cmd_dir, "ETL")
list.files(cmd_etl_dir)
```

# Automated ETL Pipeline

**⚠️ New in 2024**: The cMD ETL process has been refactored into an automated, 
modular pipeline with comprehensive validation and logging.
...
Comprehensive documentation is available in the ETL directory:

- **[README.md](ETL/README.md)**: Quick start guide and overview
- **[ARCHITECTURE.md](ETL/ARCHITECTURE.md)**: System design and data flow
- **[RUNBOOK.md](ETL/RUNBOOK.md)**: Step-by-step execution procedures
- **[TROUBLESHOOTING.md](ETL/TROUBLESHOOTING.md)**: Common issues and solutions

## Pipeline Steps

The automated pipeline consists of 7 sequential steps:

1. **01_sync_curation_maps.R** - Download curation maps from Google Sheets
2. **02_assemble_curated_metadata.R** - Combine curated data files
3. **03_build_merging_schema.R** - Create schema mapping
4. **04_build_data_dictionary.R** - Build comprehensive data dictionary
5. **05_add_dynamic_enums.R** - Add dynamic enumeration nodes
6. **06_format_for_release.R** - Format for user-facing release
7. **07_validate_and_export.R** - Validate and sync to all targets

## Key Features

- **Configuration Management**: Centralized configuration in `config.yaml`
- **Comprehensive Logging**: Structured logs with multiple levels
- **Validation Framework**: Automated validation at each step
- **Provenance Tracking**: Full audit trail of execution
- **Error Handling**: Graceful error handling with detailed messages
- **GitHub Actions**: Automated weekly execution

## Manual Execution (Legacy)

The sections below document the manual, step-by-step execution process. This is maintained for reference and backwards compatibility, but the automated pipeline (above) is recommended for routine execution.
...
assign appropriate ontology terms to original values. These tables contain 
four main columns with some additional columns depending on a attribute.

Four main columns are labeled as `original_value`, `curated_ontology`, 
`curated_ontology_term_id`, and `curated_ontology_term_db`.

The script below (`1_sync_curation_map.R`) syncs all the cMD curation maps from 
Google Sheet to the `cmd_maps_dir` (a directory under the local git project).
...
The resulting table from the below script, `template_dd`, contains all the 
columns in the final version of cMD sampleMetadata (under the `col.name` column).
...
gd_dict <- gd_dict |> dplyr::select(all_of(dd_columns))
if (ncol(gd_dict) != length(dd_columns)) {stop("Double-check subset")}
```
...
## Save
Save cMD data dictionary schema
```{r eval=FALSE}
## GitHub project
extDir <- "~/OmicsMLRepo/OmicsMLRepoData/inst/extdata"
...
## curatedMetagenomicDataCuration extdata
file.copy(from = file.path(extDir, "cMD_data_dictionary.csv"),
          to = "~/Projects/curatedMetagenomicDataCuration/inst/extdata/cMD_data_dictionary.csv",
          overwrite = TRUE)
```
