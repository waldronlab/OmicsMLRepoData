...
The script below (`1_sync_curation_map.R`) syncs all the cMD curation maps
from Google Sheet to the `cbioportal_maps_dir` (a directory under the local git 
project).
...
# Data dictionary
## Initial assembly
The resulting table from the below script, `template_dd`, contains
all the columns in the final version of cMD sampleMetadata (under the 
`col.name` column). 
...
Minor, extra updates for kept subset of sampleMetadata
```{r}
## Description for `sequencing_platform`
...
This script populate the dynamic enum nodes for the selected attributes. 
Target attributes are inside the script, so if you want to add/remove target
attributes to extract dynamic enum nodes for, make a modification in this
script. 
