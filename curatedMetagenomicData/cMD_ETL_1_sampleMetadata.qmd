...
a <- merging_schema$original_fields 
original_fields_name <- sapply(a, function(x) {
    strsplit(x, split = ";") %>% 
        unlist %>% 
        paste0("legacy_", .) %>% 
        paste0(., collapse = ";")
}) %>% as.vector
original_fields_name[which(original_fields_name == "legacy_NA")] <- NA

original_fields_completeness <- checkCurationStats(
    fields_list = original_fields_name,
...
curated_fields_name <- sapply(b, function(x) {
    strsplit(x, split = ";") %>% 
        unlist %>% 
        # paste0("curated_", .) %>% 
        gsub("curated_", "", .) %>%
