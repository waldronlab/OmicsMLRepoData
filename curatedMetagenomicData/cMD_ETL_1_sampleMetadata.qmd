...
```{r}
schema <- read_csv(file.path(dir, "inst/extdata/cMD_merging_schema.csv"))
...
formated_ori <- cbind(allSampleMetadata[,-legacy_ind],
                      allSampleMetadata[,legacy_ind]) # 'legacy_' fields at the end

curated_ind <- grep("^curated_|^original_", colnames(allSampleMetadata))
colnames(allSampleMetadata)[curated_ind] <- gsub("^curated_|original_", 
                                                 "", 
                                                 colnames(allSampleMetadata)[curated_ind])
updated <- relocate(allSampleMetadata, colnames(allSampleMetadata)[curated_ind], .after = subject_id)
updated <- relocate(allSampleMetadata, colnames(allSampleMetadata)[legacy_ind], .before = last_modified)
...
write.csv(merging_schema, 
          file.path(dir, "cMD_merging_schema.csv"), 
          row.names = FALSE)
```
