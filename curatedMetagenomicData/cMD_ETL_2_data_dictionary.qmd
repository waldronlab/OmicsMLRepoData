...
- [Update] Automatically select the common ancester for dynamic enum and update the allowed values
...
Manual update for dynamic enum: Age
```{r}
ind <- curated_age_all$col.name == "age_unit"
curated_age_all$dynamic_enum[ind] <- "NCIT:C50400"
curated_age_all$dynamic_enum_property[ind] <- "descendent"

ind <- curated_age_all$col.name == "age_group"
curated_age_all$dynamic_enum[ind] <- "NCIT:C20587"
curated_age_all$dynamic_enum_property[ind] <- "descendent"
...
Manual update for dynamic enum: Age
```{r}
curated_bodysite$dynamic_enum <- "UBERON:0001062"
curated_bodysite$dynamic_enum_property <- "descendent"
...
```{r}
curated_all <- dplyr::bind_rows(list(curated_age_all, 
                                     curated_disease_all, 
                                     curated_treatment,
                                     curated_bodysite,
                                     curated_bodysubsite,
...
updated <- dplyr::bind_rows(curated_all, formated_ori)
