...
bodysites <- unique(c(sampleMetadata$body_site,
                      ...
                      sampleMetadata$dental_sample_type,
                      sampleMetadata$disease_location))
...
bodysite_tb <- sampleMetadata[,c("curation_id", "body_site", "body_subsite",
                                 "dental_sample_type", "disease_location")]
curated_bodysite <- bodysite_tb %>%
    transmute(curation_id = curation_id,
              ...
              curated_body_site_ontology_term_id = mapvalues(x = bodysite_tb$body_site,
                                                             ...
                                                             warn_missing = TRUE),
              body_subsite = mapvalues(x = bodysite_tb$body_subsite,
                                               from = bodysite_map$original_value,
                                               to = bodysite_map$curated_ontology_term,
                                               warn_missing = TRUE),
              curated_body_subsite_ontology_term_id = mapvalues(x = bodysite_tb$body_subsite,
                                                                ...
                                                                to = basename(bodysite_map$curated_ontology_term_id),
                                                                warn_missing = TRUE),
              dental_sample_type = mapvalues(x = bodysite_tb$dental_sample_type,
                                               from = bodysite_map$original_value,
                                               to = bodysite_map$curated_ontology_term,
                                               warn_missing = TRUE),
              curated_dental_sample_type_ontology_term_id = mapvalues(x = bodysite_tb$dental_sample_type,
                                                                from = bodysite_map$original_value,
                                                                to = basename(bodysite_map$curated_ontology_term_id),
                                                                warn_missing = TRUE),
              disease_location = mapvalues(x = bodysite_tb$disease_location,
                                               from = bodysite_map$original_value,
                                               to = bodysite_map$curated_ontology_term,
                                               warn_missing = TRUE),
              curated_disease_location_ontology_term_id = mapvalues(x = bodysite_tb$disease_location,
                                                                from = bodysite_map$original_value,
                                                                to = basename(bodysite_map$curated_ontology_term_id),
                                                                warn_missing = TRUE))
## Remove body_subsite that is identical to body_site
redundant_ind <- which(curated_bodysite$curated_body_site == curated_bodysite$body_subsite)
curated_bodysite[redundant_ind, c("body_subsite", "curated_body_subsite_ontology_term_id")] <- NA
```

Add source columns for body_subsite curation and merge 3 relevant columns into
curated_body_subsite column:
```{r}
# Create a column of relevant source columns
for (i in 1:nrow(curated_bodysite)){
  curated_bodysite$curated_body_subsite_source[i] <- paste(colnames(curated_bodysite[,c(4,6,8)][which(!is.na(curated_bodysite[i,c(4,6,8)]))]), collapse=";")
}

# Convert NA values to unique character value (8X8)
curated_bodysite <- curated_bodysite %>% replace(is.na(.), "8X8") %>% data.frame()

# Merge all body_subsite value columns
for (i in 1:nrow(curated_bodysite)){
  curated_bodysite$merged[i] <- paste(curated_bodysite[i,c("body_subsite", 
                                                           "dental_sample_type", 
                                                           "disease_location")], collapse=";")
}
# Take out NA values
curated_bodysite$merged <- gsub("8X8;", "", curated_bodysite$merged)
curated_bodysite$merged <- gsub(";8X8", "", curated_bodysite$merged)
curated_bodysite$body_subsite <- curated_bodysite$merged

# Merge all body_subsite ontology id columns
for (i in 1:nrow(curated_bodysite)){
  curated_bodysite$merged[i] <- paste(curated_bodysite[i,c("curated_body_subsite_ontology_term_id", 
                                                           "curated_dental_sample_type_ontology_term_id", 
                                                           "curated_disease_location_ontology_term_id")], collapse=";")
}
# Take out NA values
curated_bodysite$merged <- gsub("8X8;", "", curated_bodysite$merged)
curated_bodysite$merged <- gsub(";8X8", "", curated_bodysite$merged)
curated_bodysite$curated_body_subsite_ontology_term_id <- curated_bodysite$merged

# Replace "8X8" values with "NA"
...
curated_bodysite <- curated_bodysite[,c(1:5, 10)]

# Rename body_subsite column
colnames(curated_bodysite)[4] <- "curated_body_subsite"
```
