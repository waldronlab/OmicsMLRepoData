...
## Antibiotic Auto-Classification and Treatment Screening

We will now use the AMR package to generate assumed names and classes for the antibiotics for curation. We will do the same for the `treatment` values and manually double-check any treatments for which antibiotic data was generated in order to assure that none of the `treatment` values are actually antibiotics.
...
A number of treatments were identified as possible antibiotics, but after manual review none were confirmed. This high false identification rate is due to the AMR package's mistake tolerance and simply indicates that the flagged treatment values appear similar to antibiotic names.
...
# create empty mapping table with antibiotic families
curated_abx <- data.frame(original_value = abx_as_abx$abx,
                          generated_name = abx_as_abx$generated_name,
                          generated_class = abx_as_abx$generated_class,
                          curated_class_value = NA,
                          curated_ontology_term = NA,
                          curated_ontology_term_id = NA)
...
curated_abx <- read_sheet(ss, sheet = "antibiotic_ontology") %>%
  select(-c(generated_name, generated_class))
...
# convert curated_ontology_term_id from url to ID
trt_map$curated_ontology_term_id <- basename(trt_map$curated_ontology_term_id)
...
# convert curated_ontology_term_id from url to ID
abx_map$curated_ontology_term_id <- basename(abx_map$curated_ontology_term_id)
```
...
Then we will create our table of curated treatment data preserving original, adjusted, and curated data. Two final tables are created for review; one with given values as well as curated values, and the other with only curated values. The table 
...
# map treatment data to curated terms and IDs
...
# select columns for final complete table
final_complete <- combined_curated %>%
  select(curation_id,
...
write.csv(final_curated, 
          file = file.path(proj_dir, "data/curated_antibiotics_treatment.csv"),
          row.names = FALSE)
```