...
information from the original diet column to produce the curated 
dietary_restriction column.
...
diet_cols <- samplemd %>% select(curation_id, diet)
...
diet_cols <- diet_cols %>% mutate(curated_dietary_restriction = diet)

# Create a column of relevant source columns
for (i in 1:nrow(diet_cols)){
  diet_cols$curated_dietary_restriction_source[i] <- ifelse(!is.na(diet_cols[i,2]), colnames(diet_cols)[2], NA)
}

# Rename original column for clarity
diet_cols <- diet_cols %>% dplyr::rename(original_dietary_restriction = diet)
```
