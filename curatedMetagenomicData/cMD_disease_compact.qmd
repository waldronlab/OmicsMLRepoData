---
title: "Clean-up the disease/age/bodysite of cMD sample metadata"
author: "Sehyun Oh"
...
```{r}
## Our curation file
file_url <- "https://raw.githubusercontent.com/waldronlab/OmicsMLRepoData/master/curatedMetagenomicData/maps/cMD_condition_ontology.csv?token=GHSAT0AAAAAAB7ODWEIAR2LACWJGB2M2KWKZALPIQA"
mlRepo_disease_all <- read.csv(file_url, sep = ",", header = TRUE)
head(mlRepo_disease_all, 3)

mlRepo_disease <- mlRepo_disease_all[,c("Original.Abbreviation.Name",
                                        "Full.Name.Curated",
                                        "Ontology.link")]
colnames(mlRepo_disease)[2] <- "Full.Name" # To merge with the other tables
```
...
condition_map <- read.csv(file.path(proj_dir, "maps/cMD_condition_ontology.csv"), 
                          sep = ",", header = TRUE)
condition_map$Ontology.link <- basename(condition_map$Ontology.link)
```

```{r}
## Subset the condition/disease-related columns
condition_tb <- sampleMetadata[,c("curation_id", "disease", 
                                  "disease_subtype", "study_condition")]
combine_condition <- function(x) {
    y <- x[c("disease", "disease_subtype", "study_condition")]
    z <- plyr::mapvalues(unlist(y), 
                         from = condition_map$Original.Abbreviation.Name,
                         to = condition_map$Full.Name.Curated,
                         warn_missing = FALSE)
...
combine_condition_ontology <- function(x) {
    ...
    z <- plyr::mapvalues(unlist(y), 
                         from = condition_map$Original.Abbreviation.Name,
                         to = condition_map$Ontology.link,
                         warn_missing = FALSE)
...
curated_condition <- condition_tb %>%
    transmute(curation_id = curation_id,
              curated_condition = apply(., 1, combine_condition),
              curated_condition_ontology = apply(., 1, combine_condition_ontology))
...
than 365. However, the final `curated_age`
```{r}
...
age_group_ontology <- data.frame(age_category = NA,
                                 curated_age_group = NA,
                                 curated_age_group_ontology = NA)
...
curated_age_group <- plyr::mapvalues(x = age_group_only$age_category, 
                                     from = age_group_map$age_category, 
...
setdiff(bodysites, cMD_bodysite$Original.Abbreviation.Name)
```
...
curated_bodysite <- bodysite_tb %>%
    transmute(curation_id = curation_id,
              curated_body_site = mapvalues(x = bodysite_tb$body_site,
                                            from = bodysite_map$Original.Abbreviation.Name,
                                            to = bodysite_map$Ontology.Name,
                                            warn_missing = TRUE),
              curated_body_site_ontology = mapvalues(x = bodysite_tb$body_site,
                                                    from = bodysite_map$Original.Abbreviation.Name,
                                                    to = bodysite_map$Ontology.link,
                                                    warn_missing = TRUE),
              curated_body_subsite = mapvalues(x = bodysite_tb$body_subsite,
                                               from = bodysite_map$Original.Abbreviation.Name,
                                               to = bodysite_map$Ontology.Name,
                                               warn_missing = TRUE),
              curated_body_subsite_ontology = mapvalues(x = bodysite_tb$body_subsite,
                                                        from = bodysite_map$Original.Abbreviation.Name,
                                                        to = bodysite_map$Ontology.link,
                                                        warn_missing = TRUE))
...
curated_bodysite[redundant_ind, c("curated_body_subsite", "curated_body_subsite_ontology")] <- NA
```
