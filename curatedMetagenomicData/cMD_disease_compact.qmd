...
```{r}
file_url <- "https://github.com/mcarlsn/Bioinformatics_Data_Curation_Collaboration/blob/main/disease_ontology.yaml"
...
# cmd_disease_unique_disease %>% rename("Full.Name.Curated" = "Full.Name")
# Error: Error in rename(., Full.Name.Curated = "Full.Name") : Some 'from' names in value not found on 'x': Full.Name.Curated
```
...
```{r}
(ss <- gs4_create("cMD metadata ontology_NeedsCuration", sheets = manaul_curation))
```

# Temporal file for manual curation
Export the merged mapping table to Google Sheet for manual curation.
```{r}
(ss <- gs4_create("cMD metadata ontology_DRAFT", sheets = cmd_disease_unique_disease))
```

# Save manually curated mapping table 
After the manual curation, update the target mapping table in GitHub. 
```{r}
manual_curation <- read_sheet("https://docs.google.com/spreadsheets/d/1TOcV_T6gvtbs0KTC9m_jCdbEST2QTFsfXb3RF-VXu9I/edit?usp=sharing")
```
