...
disease_ontology <- tibble::rownames_to_column(y, "Original.Abbreviation.Name")
...
We subset `cmd_disease_unique_disease` with the unique values that also have 
the `Ontology.link` value. This table is named `ont_complete`. 
```{r}
ont_complete <- cmd_disease_unique_disease %>% 
...
manaul_curation <- cmd_disease_unique_disease %>% 
    distinct(Original.Abbreviation.Name, .keep_all=TRUE) %>% 
    filter(Ontology.link == "")

# Add a new column for "Revision Required" that explains observations that require clarification
manaul_curation <- manaul_curation %>% 
    add_column(Revision_Required = "")
```
