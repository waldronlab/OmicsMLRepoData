...
check_infant <- rowSums(sampleMetadata$infant_age/365 > sampleMetadata$age+1, na.rm = TRUE)
# There are 14 observations flagged after running this line 

sampleMetadata %>% filter(sampleMetadata$infant_age/365 > sampleMetadata$age+1)
# Of the 14 observations that do not pass our sanity check, examples include rows where age = 0 and infant age = 420, 372, 371, etc. This may not be an issue because in our converted age columns, infant_age is divided by 365 to convert to year values. Correct age per observation is still captured. 
```
