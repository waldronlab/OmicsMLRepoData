...
for (i in seq_len(nrow(condition_map))) {
    onto_term <- condition_map$original_value[i]
    onto_id <- basename(condition_map$Ontology.link[i])
...
combine_condition_ontology <- function(x) {
    ...
    z <- plyr::mapvalues(unlist(y), 
                         from = condition_map$original_value,
                         to = condition_map$curated_ontology_term_id,
                         warn_missing = FALSE)
...
curated_bodysite <- bodysite_tb %>%
    transmute(curation_id = curation_id,
              ...
              curated_body_site_ontology = mapvalues(x = bodysite_tb$body_site,
                                                    from = bodysite_map$original_value,
                                                    to = bodysite_map$curated_ontology_term_id,
                                                    warn_missing = TRUE),
