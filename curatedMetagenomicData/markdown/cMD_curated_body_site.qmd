---
title: "Harmonize body site information in cMD sample metadata"
author:
  - Sehyun Oh
  - Kai Gravel-Pucillo
...
This file curates information from the original body_site column to produce 
the curated body_site column.
...
# mapped_terms <- read_sheet(ss, sheet = "body_site_ontology")

# import body_site_detailed ontology map
mapped_terms <-read.csv(file = file.path(proj_dir, "cMD_body_site_map.csv"), header=T)
setdiff(unique_merged_vals$body_site, mapped_terms$original_value)
```
...
for (i in 1:length(bodysub_cols$body_site)){
  if (!is.na(bodysub_cols$body_site[i])){
    new_term <- mapped_terms$curated_ontology_term[grep(paste("^",bodysub_cols$body_site[i],"$",sep=""), mapped_terms$original_value, fixed=F)]
    new_term_id <- mapped_terms$curated_ontology_term_id[grep(paste("^",bodysub_cols$body_site[i],"$",sep=""), mapped_terms$original_value, fixed=F)]
    # Create curated value
    bodysub_cols$curated_body_site[i] <- new_term
    bodysub_cols$curated_body_site_ontology_term_id[i] <- new_term_id
  }else{
    bodysub_cols$curated_body_site[i] <- NA
    bodysub_cols$curated_body_site_ontology_term_id[i] <- NA   
  }
}
```
...
for (i in 1:nrow(bodysub_cols)){
  bodysub_cols$curated_body_site_source[i] <- ifelse(is.na(bodysub_cols$body_site[i]), NA, "body_site")
}

# Create a curated dataframe
cur_body_site <- bodysub_cols[,c(1, 3:6)]

cur_body_site <- cur_body_site %>% dplyr::rename(original_body_site = body_site)

# Replace empty values from curated columns with "NA"
cur_body_site <- data.frame(lapply(cur_body_site, 
...
# export to GitHub
write.csv(cur_body_site, 
          file = file.path(proj_dir, "curated_body_site.csv"),
          row.names = FALSE)
```