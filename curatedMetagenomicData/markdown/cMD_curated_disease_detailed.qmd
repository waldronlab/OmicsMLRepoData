---
title: "Harmonize disease_detailed information in cMD sample metadata"
author:
  - Sehyun Oh
  - Kai Gravel-Pucillo
...
disease_detailed data with the curatedMetagenomicData package.
...
disease_detailed_map <- read_sheet(ss, sheet = "disease_detailed_ontology")

# create validation table for curation statuses
sc_validation_tb <- data.frame(matrix(NA, nrow = nrow(disease_detailed_map), ncol = 4))
...
for (i in seq_len(nrow(disease_detailed_map))) {
    onto_term <- disease_detailed_map$curated_ontology_term[i]
    onto_id <- basename(disease_detailed_map$curated_ontology_term_id_link[i])
...
disease_detailed_ontology <- read_sheet(ss, sheet = "disease_detailed_map")

# save map to GitHub
write.csv(disease_detailed_ontology,
          file = file.path(proj_dir, "maps/cMD_disease_detailed_map.csv"),
          row.names = FALSE)
```
...
disease_detailed_ontology_map <- read.csv(file = file.path(proj_dir, "maps/cMD_disease_detailed_map.csv"),
                          sep = ",", header = TRUE)
```

## Curating "disease_detailed" Column
...
for (x in 1:length(disease_data$merged)){
  ...
  for (i in 1:length(original_terms)){
    new_term <- disease_detailed_ontology_map$curated_ontology_term[grep(paste("^",original_terms[i],"$",sep=""), disease_detailed_ontology_map$original_value, fixed=F)]
    new_term_id <- disease_detailed_ontology_map$curated_ontology_term_id[grep(paste("^",original_terms[i],"$",sep=""), disease_detailed_ontology_map$original_value, fixed=F)]
  ...
  disease_data$curated_disease_detailed[x] <- paste(new_terms, collapse= ";")
  disease_data$curated_disease_detailed_ontology_term_id[x] <- paste(new_term_ids, collapse= ";")
  }
}
...
for (i in 1:nrow(disease_sources)){
  disease_data$curated_disease_detailed_source[i] <- paste(colnames(disease_sources[which(!is.na(disease_sources[i,]))]), collapse=";")
}
```
...
disease_cols <- c("merged",
                  "curated_disease_detailed", 
                  "curated_disease_detailed_ontology_term_id", 
                  "curated_disease_detailed_source")
...
curated_study_condition <- curated_study_condition %>% rename(original_disease_detailed = merged)
...
write.csv(curated_study_condition, 
          file = file.path(proj_dir, "data/curated_disease_detailed.csv"),
          row.names = FALSE)
```