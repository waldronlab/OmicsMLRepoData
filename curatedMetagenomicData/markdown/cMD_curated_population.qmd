---
title: "Harmonize population information in cMD sample metadata"
author:
  - Sehyun Oh
  - Kai Gravel-Pucillo
...
This .qmd file demonstrates a workflow for curating and harmonizing population data 
with the curatedMetagenomicData package.
This file curates information from the original population column to 
produce the curated population column.
...
# import population ontology map
mapped_terms <-read.csv(file = file.path(proj_dir, "cMD_population_map.csv"), header=T)
```


# Curating Population Column
...
for (x in 1:length(population_cols$merged)){
  ...
  population_cols$curated_population[x] <- rmv_xtra_semis(paste(as.list(unique(new_terms)), collapse= ";"))
  population_cols$curated_population_ontology_term_id[x] <- rmv_xtra_semis(paste(as.list(unique(new_term_ids)), collapse= ";"))
...
for (i in 1:nrow(population_cols)){
  population_cols$curated_population_source[i] <- ifelse(is.na(population_cols[i,2]), NA, colnames(population_cols)[2])
}
...
colnames(curated_population)[2] <- "original_population"
...
write.csv(curated_population, 
          file = file.path(proj_dir, "curated_population.csv"),
          row.names = FALSE)
```