---
title: "Harmonize population ancestry detailed information in cMD sample metadata"
author:
  - Sehyun Oh
  - Kai Gravel-Pucillo
...
ancestry detailed data with the curatedMetagenomicData package.
This file curates information from the original population column to 
produce the curated population ancestry detailed column.
...
write.csv(unique_merged_vals, 
          file= file.path(proj_dir, "population_detailed_cols_unique_vals.csv"), 
...
`population_ancestry_detailed_ontology_map`.
```{r import_maps, eval=FALSE}
# import population detailed ontology map
# mapped_terms <- read_sheet(ss, sheet = "population_map")

# import population detailed ontology map
mapped_terms <-read.csv(file = file.path(proj_dir, "cMD_population_ancestry_detailed_map.csv"), header=T)
```


# Curating Population Ancestry Detailed Column
...
for (x in 1:length(population_cols$merged)){
  ...
  population_cols$curated_population_ancestry_detailed[x] <- rmv_xtra_semis(paste(as.list(unique(new_terms)), collapse= ";"))
  population_cols$curated_population_ancestry_detailed_ontology_term_id[x] <- rmv_xtra_semis(paste(as.list(unique(new_term_ids)), collapse= ";"))
...
for (i in 1:nrow(population_cols)){
  population_cols$curated_population_ancestry_detailed_source[i] <- ifelse(is.na(population_cols[i,2]), NA, colnames(population_cols)[2])
}
...
colnames(curated_population)[2] <- "original_population_ancestry_detailed"
