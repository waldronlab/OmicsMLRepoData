...
disease_subtype columns to produce the curated control, target_condition, and
disease columns.
...
(`study_name:sample_id`) to avoid confusion due to duplicated samples. Then 
we will set up a Google Sheet to export our mapping table to.
...
study_condition_ontology <- read_sheet(ss, sheet = "disease_ontology_all")
...
Using the "study_condition" column as a guide, identify whether each sample 
was part of the "control" group or the "case" group for its corresponding study. 
Samples that were not utilized in the study receive the label "Not Used".
```{r}
...
for (i in 1:nrow(disease_data)){
  if (grepl("control", disease_data$study_condition[i])){
    ...
    disease_data$control_ontology_term_id[i] <- "NCIT:C142703" # http://purl.obolibrary.org/obo/NCIT_C142703
  }
  else if (is.na(disease_data$study_condition[i])){
    disease_data$control[i] <- "Not Used"
    disease_data$control_ontology_term_id[i] <- "NCIT:C69062" # http://purl.obolibrary.org/obo/NCIT_C69062
  }
  else{
    disease_data$control[i] <- "Case"   
    disease_data$control_ontology_term_id[i] <- "NCIT:C49152" # http://purl.obolibrary.org/obo/NCIT_C49152
  }
}
```

Now we can clean up the "study_condition" column by replacing any instances 
of the term "control" with an "NA" value, which will prevent redundant data 
when we combine this column with the "disease" column.
...
for (i in 1:length(sampleMetadata$study_name)){
  disease_data$curated_target_condition[i] <- target_condition_map$target_condition[which(target_condition_map$study_name == sampleMetadata$study_name[i])]
  disease_data$curated_target_condition_ontology_term_id[i] <- target_condition_map$target_condition_term_id[which(target_condition_map$study_name == sampleMetadata$study_name[i])]
}
```
...
curated_study_condition <- dplyr::right_join(
    sampleMetadata[, c("curation_id", "study_condition", "disease", "disease_subtype")], 
    disease_data[, c("curation_id", "control", "control_ontology_term_id",
                     "curated_target_condition", "curated_target_condition_ontology_term_id", 
                     "curated_disease", "curated_disease_ontology_term_id")], 
    by = "curation_id")
...
```{r}
## Double-check whether the 'one-to-one' mapping update in study_condition map
## is properly applied during the mapping step. 
val <- sapply(curated_study_condition$curated_target_condition, 
              function(x) {
                  strsplit(x, split = ";") %>% unlist %>% length
              }) %>% as.numeric()
onto <- sapply(curated_study_condition$curated_target_condition_ontology_term_id, 
               function(x) {
                   strsplit(x, split = ";") %>% unlist %>% length
               }) %>% as.numeric()
